...
static void SpawnEntityPlaceholder(Tachyon* tachyon, State& state, GameEntity& entity) {
  ...
  // @todo allow entity placeholder default material
  {
    ...
    if (
      entity.type == FOG_SPAWN ||
      entity.type == EVENT_TRIGGER ||
      entity.type == VANTAGE_SPOT
    ) {
...
static tVec3f GetSpawnPosition(Tachyon* tachyon, State& state) {
  ...
  if (editor.is_placing_entity) {
    ...
    if (
      ...
      entity_type == LILAC_BUSH ||
      entity_type == ROSE_BUSH
    ) {
      auto& defaults = GetEntityDefaults(entity_type);
...
void LevelEditor::OpenLevelEditor(Tachyon* tachyon, State& state) {
  ...
  objects(meshes.astrolabe_plate2).disabled = true;
  objects(meshes.astrolabe_plate3).disabled = true;
  objects(meshes.astrolabe_plate4).disabled = true;
...
void LevelEditor::CloseLevelEditor(Tachyon* tachyon, State& state) {
  ...
  objects(meshes.astrolabe_plate2).disabled = false;
  objects(meshes.astrolabe_plate3).disabled = false;
  objects(meshes.astrolabe_plate4).disabled = false;
