  ...
  behavior StarFlower {
    ...
    timeEvolve() {
      ...
      for_entities(state.starflowers) {
        ...
        // Leaves
        {
          auto& leaves = objects(meshes.starflower_leaves)[i];
        ...
        // Petals
        {
          auto& petals = objects(meshes.starflower_petals)[i];

          Sync(petals, entity);

          float emissivity = state.is_nighttime ? 0.5f : 0.2f;

          petals.color = tVec4f(0.8f, 1.f, 1.f, emissivity);
