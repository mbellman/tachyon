...
#include "astro/entity_behaviors/CastleArch.h"
...
#include "astro/entity_behaviors/CastleWallFountain.h"
...
#include "astro/entity_behaviors/Signpost.h"
...
#include "astro/entity_behaviors/Vine.h"
...
#define create_dispatch_cases(dispatch_macro)\
  ...
  dispatch_macro(CASTLE_ARCH, CastleArch);\
  dispatch_macro(CASTLE_RAMPART, CastleRampart);\
  dispatch_macro(CASTLE_STAIRS, CastleStairs);\
  dispatch_macro(CASTLE_STEEPLE, CastleSteeple);\
  dispatch_macro(CASTLE_TOWER, CastleTower);\
  dispatch_macro(CASTLE_WALL_FOUNTAIN, CastleWallFountain);\
  ...
  dispatch_macro(SIGNPOST, Signpost);\
...
std::vector<GameEntity>& EntityDispatcher::GetEntityContainer(State& state, EntityType type) {
  switch (type) {
    ...
    dispatch_GetEntityContainer(CASTLE_ARCH, state.castle_arches);
    dispatch_GetEntityContainer(CASTLE_RAMPART, state.castle_ramparts);
    dispatch_GetEntityContainer(CASTLE_STAIRS, state.castle_stairs);
    ...
    dispatch_GetEntityContainer(CASTLE_WALL_FOUNTAIN, state.castle_wall_fountains);
    ...
    dispatch_GetEntityContainer(SIGNPOST, state.signposts);
    ...
    dispatch_GetEntityContainer(VINE, state.vines);
